...
There are **192 registered tools / 22 categories**, including **64 confirmed action tools**. Domain files are under `Tools/Pdm`, `Entities`, `Documents`, `License`, `Sketch2D`, `Sketch3D`, `Design2D`, `Design3D`, `Assembly`, `Tooling`, `Drafting`, `Electrode`, `Cae`, and `Cam/*`. Shared schema, result conversion and registration avoid duplication. Batch tool files sit alongside each domain; native sketch/shape construction lives in `Automation/SketchBatchChanges.cs` and `ShapeBatchChanges.cs`. See [batch scope and evidence](../docs/BATCH_TOOLS.md).

The complete coverage report and generated tool schemas are built from a machine-local SDK/reference cache and are not published here. Every bound operation is a static call compiled against the vendor SDK. No reflection dispatcher or arbitrary API invocation exists.

CAM technology folders preserve the requested organization. Existing operations are inspected and individually calculated through shared public CAM services. Scalar CAM parameter changes require exact names/types and confirmation. The documented `ISimulation`/`IVerify` workflows can start simulation for one operation or verification for one/all operations; these actions require confirmation and do not generate NC, run a machine, or certify collision safety. Strategy creation and NC postprocessing/transmission remain unexposed. Wire and standalone PDM Explorer references are cached; their host adapters are not implemented. PDM tools use `TopSolidHost.Pdm`.
