...
| Profiles and sections: Automation Guide pp.39–42; User's Guide pp.92–93 | Native line/arc contour creation, append to a sketch, explicit section creation, returned topology handles | Closed contours repeat their starting point. Since 0.5.1, `createSection=false` is the default; sections are an explicit modeling option. Native closure and length are checked; input and output segment counts need not match. |
...
| CAM creation/simulation examples: CAM Guide pp.9–10 | Guide reviewed; shared CAM operation services retained in domain folders | Static operation creation, simulation/verification execution, postprocessing and machine/NC execution are not exposed in this version. |
...
- “Create an 80 by 50 mm rectangle in the active part and extrude it by 20 mm.” The model obtains the current document, creates a sketch, passes its returned section to extrusion, and uses the new revision IDs.
